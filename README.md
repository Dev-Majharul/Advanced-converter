...
```bash
# Convert an image
curl -X POST -F "file=@image.jpg" -F "format=png" -F "quality=90" \
  http://localhost:5000/api/convert/image

# Convert a video
curl -X POST -F "file=@video.mp4" -F "format=webm" -F "resolution=720p" \
  http://localhost:5000/api/convert/video
...
- **Node version error**: Update to Node.js 14+ or use NVM to manage Node versions
- **Permission errors**: Try running with administrator/sudo privileges

### Conversion Issues

- **Conversion failed**: Check that input file is not corrupted or password-protected
- **Poor quality output**: Increase the quality setting or try a different format
- **Slow conversions**: Close other CPU-intensive applications, or adjust thread count in settings

### System Requirements

- **Minimum**: Dual-core CPU, 4GB RAM, 100MB free disk space
- **Recommended**: Quad-core CPU, 8GB RAM, 500MB free disk space plus adequate space for your files
